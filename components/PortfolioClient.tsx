      ...
      <div className="hv-a-hero">
        <div className="hv-a-left">
          ...
          <div className="hv-a-trust">
            <div><b>4.9★</b><span>target rating</span></div>
      ...
      <div className="dn-a-hero">
        <div className="dn-a-left">
          ...
          <div className="dn-a-trust">
            ...
            <div className="dn-a-trust-text"><b>Real Google reviews</b><span>Insurance billed for you · Spanish &amp; English</span></div>
          </div>
        </div>
      ...
      <div className="pl-a-hero">
        <div className="pl-a-left">
          ...
          <div className="pl-a-trust">
            ...
            <div className="pl-a-trust-item"><b>5.0★</b><span>target rating</span></div>
            <div className="pl-a-divider"></div>
      ...
      <div className="rf-a-hero">
        <div className="rf-a-left">
          ...
          <div className="rf-a-trust">
            <div className="rf-a-trust-item"><div className="rf-a-trust-num">Hundreds</div><div className="rf-a-trust-cap">of roofs installed</div></div>
        ...
        <div className="rf-a-right">
          <div className="rf-a-quote-card">
            ...
            <p>&quot;Sample testimonial — this is how a featured customer review would look in the final design.&quot;</p>
            <div className="rf-a-q-author">— Illustrative customer</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="ls-a-hero">
        <div className="ls-a-left">
          ...
          <div className="ls-a-trust">
            <div className="ls-a-trust-item"><span className="ls-a-trust-num">Hundreds</span><span className="ls-a-trust-cap">of Houston yards transformed</span></div>
            <div className="ls-a-trust-item"><span className="ls-a-trust-num">4.9★</span><span className="ls-a-trust-cap">target rating</span></div>
...
function getBusinesses(isEn: boolean) {
  return [
    ...
    { id:'plumbing',    num:'03', name:'Fast Flow Plumbing',   industry: isEn ? 'Plumbing' : 'Plomería',    url:'fastflowplumbing.com',   tab:'Fast Flow Plumbing — 24/7 Emergency',       Before:PlumbingBefore,    After:PlumbingAfter },
...
function Card({ biz, isEn }: { biz: ReturnType<typeof getBusinesses>[0]; isEn: boolean }) {
  const [mode, setMode] = useState<'before'|'after'>('before')
  const Before = biz.Before
  const After = biz.After
  const beforeLabel = isEn ? 'Before' : 'Antes'
  const afterLabel = isEn ? 'After' : 'Después'
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'var(--muted)', padding:'0 4px' }}>
        <span>{mode==='before'
          ? (isEn ? 'Current site — legacy, no strategy.' : 'Sitio actual — heredado, sin estrategia.')
          : (isEn ? 'CECA redesign — branding, CRO, tracking.' : 'Rediseño CECA — branding, CRO, tracking.')}</span>
        <span style={{ display:'inline-flex', alignItems:'center', padding:'4px 10px', background:'var(--fg)', color:'var(--bg)', borderRadius:999, fontFamily:'var(--mono)', fontWeight:600, fontSize:10, letterSpacing:'.1em', textTransform:'uppercase' }}>{mode==='before' ? beforeLabel : afterLabel}</span>
      </div>
    </div>
  )
}

export default function PortfolioClient({ lang }: { lang: 'en' | 'es' }) {
  const isEn = lang === 'en'
  const t = CECA_CONTENT[lang] || CECA_CONTENT.en
  const online = isEn ? 'ONLINE' : 'EN LÍNEA'
  const businesses = getBusinesses(isEn)
  const homePath = isEn ? '/' : '/es'
      ...
      <div style={{ minHeight:'100vh', background:'var(--bg)', color:'var(--fg)', paddingTop:84 }}>
        <section style={{ padding:'80px 28px 64px', borderBottom:'1px solid var(--line)' }}>
          <div style={{ maxWidth:1320, margin:'0 auto' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingBottom:14, borderBottom:'1px solid var(--line)', marginBottom:40, fontFamily:'var(--mono)', fontSize:11, color:'var(--muted)', letterSpacing:'.1em', textTransform:'uppercase' }}>
              <span><span style={{ color:'var(--accent)', marginRight:8 }}>●</span>// {isEn ? 'Portfolio' : 'Portafolio'}</span>
              <Link href={homePath} style={{ color:'var(--muted)' }}>{isEn ? '← Back to home' : '← Volver al inicio'}</Link>
            </div>
            <h1 style={{ margin:0, fontFamily:'var(--display)', fontWeight:900, fontSize:'clamp(40px, 6vw, 96px)', lineHeight:0.92, letterSpacing:'-.035em' }}>
              {isEn
                ? <>Before and<br/><span style={{ color:'var(--accent)', fontStyle:'italic' }}>after.</span></>
            ...
            <p style={{ marginTop:28, fontSize:18, lineHeight:1.55, maxWidth:580, color:'var(--muted)' }}>
              {isEn
                ? 'Five Houston businesses redesigned from the ground up. Each with its own voice and strategy — with the same goal: turning visits into calls.'
            ...
            <div style={{ marginTop:16, fontFamily:'var(--mono)', fontSize:12, color:'var(--muted)', letterSpacing:'.06em', textTransform:'uppercase' }}>
              {isEn ? 'Toggle each card to compare →' : 'Activa el toggle en cada tarjeta para comparar →'}
            </div>
            <div style={{ marginTop:20, padding:'12px 18px', background:'var(--panel)', border:'1px solid var(--line)', borderRadius:8, display:'inline-block', fontFamily:'var(--mono)', fontSize:12, color:'var(--muted)', letterSpacing:'.02em' }}>
              {isEn
                ? 'ⓘ Conceptual examples — they illustrate our level of design and strategy, not real client sites.'
                : 'ⓘ Ejemplos conceptuales — ilustran nuestro nivel de diseño y estrategia, no son sitios de clientes reales.'}
            </div>
          </div>
        </section>
        ...
        <section style={{ padding:'80px 28px 120px' }}>
          <div style={{ maxWidth:1320, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'56px 40px' }}>
            {businesses.map(b => <Card key={b.id} biz={b} isEn={isEn} />)}
          </div>
        </section>
        ...
        <section style={{ padding:'64px 28px', borderTop:'1px solid var(--line)', background:'var(--panel)' }}>
          <div style={{ maxWidth:1320, margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:24 }}>
            <div>
              <h2 style={{ margin:0, fontFamily:'var(--display)', fontWeight:900, fontSize:'clamp(28px,4vw,48px)', letterSpacing:'-.02em' }}>
                {isEn ? 'Is your business next?' : '¿Tu negocio es el próximo?'}
              </h2>
              <p style={{ margin:'8px 0 0', color:'var(--muted)', fontSize:16 }}>
                {isEn ? 'Free audit — we respond in under 24 hours.' : 'Auditoría gratis — respondemos en menos de 24 horas.'}
              </p>
            </div>
            <Link href={contactPath} style={{ background:'var(--accent)', color:'var(--accent-contrast)', padding:'18px 28px', fontFamily:'var(--mono)', fontSize:13, fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase' }}>
